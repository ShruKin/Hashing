...
//  division method hashing
struct HashTable insert_hash_div_dh(struct HashTable ht, int data){
    int key;
    for(int i=0; i<ht.size; i++){
        key = dh_div(data, i, ht.size);
        if(ht.table[key].is_vacant){
            ht.table[key].data = data;
            ht.table[key].is_vacant = 0;
            break;
...
//  multiplication method hashing
struct HashTable insert_hash_multi_dh(struct HashTable ht, int data){
    int key;
    for(int i=0; i<ht.size; i++){
        key = dh_multi(data, i, ht.size);
        if(ht.table[key].is_vacant){
            ht.table[key].data = data;
            ht.table[key].is_vacant = 0;
            break;
...
//  division method hashing
int search_hash_div_dh(struct HashTable ht, int data){
    int key;
    for(int i=0; i<ht.size; i++){
        key = dh_div(data, i, ht.size);
        if(ht.table[key].is_vacant)
...
//  multiplication method hashing
int search_hash_multi_dh(struct HashTable ht, int data){
    int key;
    for(int i=0; i<ht.size; i++){
        key = dh_multi(data, i, ht.size);
        if(ht.table[key].is_vacant)
